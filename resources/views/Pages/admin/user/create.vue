<template>
    <form @submit.prevent="createUser()">
        <a-card title="Tạo mới tài khoản" style="width: 100%;">
            <div class="row mb-3">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 text-center">
                            <a-upload :before-upload="beforeUpload" @change="handleFileChange()" name="file"
                                v-model:file-list="fileList" :max-count="1" action="http://localhost:8000/api/upload/user">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-12 col-sm-5">
                            <a-radio-group v-model:value="role">
                                ...
                                <a-radio checked value="1">Người dùng</a-radio>
                            </a-radio-group>
                        </div>
                    </div>
                </div>
...
import { message, Upload, Radio } from 'ant-design-vue';
...
export default defineComponent({
    ...
    setup() {
        const avatarSrc = ref("http://localhost:8000/storage/img/users/default.png");
        const fileList = ref([]);
        const store = menu();
        ...
        const handleFileChange = () => {
            if (fileList.length > 0) {
                ...
                if (file.status === "done" && file.response && file.response.file_path) {
                    avatarSrc = file.response.file_path;
                }
            } else {
                avatarSrc.value = "http://localhost:8000/storage/img/users/default.png";
            }
        }
        ...
        const createUser = () => {
            if (fileList.value.length > 0)
                ...
                users.avatar = avatarSrc.value;
            axios.post('http://localhost:8000/api/users', users)
                .then((response) => {
                    if (response.status == 200) {
                        message.success("Thêm tài khoản thành công");
